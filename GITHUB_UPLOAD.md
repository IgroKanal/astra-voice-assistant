...
Не используй `git add .`. Сначала должны пройти автоматические тесты, ручная
Windows-проверка и независимый review с verdict `COMMIT`. После этого проверь
...
Добавляй только проверенные файлы из списка конкретного патча. Commit/push
выполняются пользователем вручную после review gate.
